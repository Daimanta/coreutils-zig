...
pub fn toLocalDateTimeStringAlloc(alloc: *std.mem.Allocator, local_time: *struct_tm) ![]const u8 {
    ...
    _ = std.fmt.bufPrintIntToSlice(result[0..4], @intCast(u32, local_time.tm_year + 1900), 10, false, std.fmt.FormatOptions{.width=4, .fill='0'});
    _ = std.fmt.bufPrintIntToSlice(result[5..7], @intCast(u32, local_time.tm_mon + 1), 10, false, std.fmt.FormatOptions{.width=2, .fill='0'});
    _ = std.fmt.bufPrintIntToSlice(result[8..10], @intCast(u32, local_time.tm_mday), 10, false, std.fmt.FormatOptions{.width=2, .fill='0'});
    _ = std.fmt.bufPrintIntToSlice(result[11..13], @intCast(u32, local_time.tm_hour), 10, false, std.fmt.FormatOptions{.width=2, .fill='0'});
    _ = std.fmt.bufPrintIntToSlice(result[14..], @intCast(u32, local_time.tm_min), 10, false, std.fmt.FormatOptions{.width=2, .fill='0'});
    result[4] = '-';
    result[7] = '-';
    result[10] = ' ';
    result[13] = ':';
